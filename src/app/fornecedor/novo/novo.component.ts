...
export class NovoComponent implements OnInit {
  ...
  configurarMascaraDocumento(): TextMaskConfig {
    return this.tipoFornecedorForm()?.value === "1" ? {mask: this.MASKS.cpf.textMask} : {mask: this.MASKS.cnpj.textMask};
  }
  ...
  buscarCep(target: any){

    let cep = StringUtils.somenteNumeros(target.value);
    console.log(cep);
    if(cep.length === 8 ) this.fornecedorService.consultarCep(cep)
      .subscribe(
        cepRetorno => this.preencherCepConsulta(cepRetorno),
        error => this.errors.push(error)
      );

  }

  preencherCepConsulta(cepConsulta: CepConsulta){
